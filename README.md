*NB: An easy way to try the software is to check out the latest version number from 4mountains_rel.html (search for the string "beta") and then type that number as X in http://a.moq.fi/testX.htm. The software is maintained only for Chrome browser.*
...
Another goal is to implement a model of the spatial world into Four Mountains. This would allow computer-controlled characters in a virtual environment to spontaneously express emotions, form friendships, have arguments, specialize in different roles, form a heterarchy, increase or reduce the social authority of each other, split into subgroups according to friendship instead of plan and so on. What kind of friendships are sought after can be affected by situational factors. Such as, if a computer-controlled character wanted to have fun it might seek the company of "party people" characters and if it wanted to learn things it might seek the company of characters who have intellectual authority.
